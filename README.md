# OmniScout

OmniScout is an autonomous AI research agent designed to perform deep information retrieval and analysis. Built with Python and powered by Google's Gemini 2.5 Flash model, it demonstrates modern agentic workflows and secure development practices.

## 🚀 Features
- **Autonomous Research:** Leverages LLMs to decompose complex queries into actionable tasks.
- **Real-time Web Access:** (Coming soon) Ability to search and synthesize information from the web.
- **Secure Architecture:** Built with security-first principles, utilizing environment variables for credential management.

## 🛠 Tech Stack
- **Language:** Python 3.14+
- **LLM:** Google Gemini 2.5 Flash
- **Framework:** LangChain (Architectural foundation for the agent)
- **Environment Management:** Python venv & python-dotenv

## ⚙️ Setup & Installation

### Prerequisites
- Python 3.10 or higher
- A Google AI Studio API Key

### Installation
1. **Clone the repository:**
   ```bash
   git clone [https://github.com/your-username/omniscout.git](https://github.com/PontusRodmalm/omniscout.git)
   cd omniscout

### Diagnostic Tools
Included is a check_models.py script to verify available Gemini model IDs and API connectivity.
